...
- **`STS_PARALLEL_WORKERS`** – Max models to run concurrently in `run_autogenerated_tests.py` (default: `2`; set to `1` for sequential).
...
- **`run_autogenerated_tests.py`** — same OpenAPI functional pipeline as `tests/test_generated/`, but **CLI** timestamped HTML/JSON under `reports/<model>/`. Runs models **in parallel** (default 2 workers; `STS_PARALLEL_WORKERS=1` for sequential). Optional: `STS_MODELS=PSDC,CTDC`.
