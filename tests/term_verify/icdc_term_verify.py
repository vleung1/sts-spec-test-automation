"""
ICDC term-by-value verification pipeline driven by vendored ``icdc-model-props.yml``.

**Same three stages as CCDI** (see :mod:`ccdi_term_verify`), but **extract** uses
:func:`clean_enum_value` (strip `` #`` comments only when the value is not quoted, then unquote)
to match ``extract_icdc_enum_properties.py`` — not :func:`~ccdi_term_verify.strip_inline_yaml_comment`.

**Enrich/verify** use :func:`verify_row` from ``ccdi_term_verify`` with ICDC model paths only.
...
def default_yaml_path() -> Path:
    """Default vendored ICDC YAML path (override with ``--yaml``)."""
    return _repo_root() / "data" / "data-models-yaml" / "icdc-model-props.yml"


def default_report_dir() -> Path:
    """Default ``reports/term_value/ICDC/`` output directory."""
    return _repo_root() / "reports" / "term_value" / "ICDC"


# --- extract (matches extract_icdc_enum_properties.py) ---
...
def clean_enum_value(val: str) -> str:
    """
    Normalize one raw enum list item from ICDC YAML for downstream CSV and STS lookup.

    Strips a trailing `` #...`` comment only when the value does **not** start with a quote (so
    values like ``"CD3/CD30 Cells, #, Blood"`` stay intact), then removes surrounding quotes.
    """
    val = val.strip()
    idx = val.find(" #")
    if idx != -1:
...
def parse_icdc_yaml_props(path: Path) -> list[tuple[str, str, list[str]]]:
    """
    Walk ICDC property YAML; return ``(prop_handle, description, enum_handles)`` per property.

    Enum list items are passed through :func:`clean_enum_value` and deduplicated in order.
    """
...
def run_extract(yaml_path: Path, out_dir: Path) -> tuple[Path, Path]:
    """Emit ``icdc_enum_properties_summary.csv`` and ``icdc_enum_terms_for_verification.csv``."""
    if not yaml_path.exists():
        raise FileNotFoundError(f"YAML not found: {yaml_path}")

    out_dir.mkdir(parents=True, exist_ok=True)
    summary_csv = out_dir / "icdc_enum_properties_summary.csv"
    query_csv = out_dir / "icdc_enum_terms_for_verification.csv"

    parsed = parse_icdc_yaml_props(yaml_path)
...
def discover_icdc_version(client: APIClient) -> str | None:
    """Latest ICDC version from ``GET /models`` or :func:`get_latest_version`."""
...
def build_prop_to_node_map(client: APIClient, version_string: str) -> dict[str, str]:
    """Map ICDC property handle → first node that declares the property."""
...
def fetch_handle_to_value_all(
    ...
    *,
    ...
    """Paginated ``GET .../terms`` for ICDC: full ``handle → value`` map for one property."""
    path = (
        f"/model/{quote(MODEL_HANDLE, safe='')}/version/{quote(version_string, safe='')}"
...
def run_enrich(client: APIClient, query_csv: Path, out_dir: Path) -> tuple[str, Path]:
    """
    Fill ``model_handle``, ``version_string``, ``node_handle``, and ``term_value``; write enriched CSV.

    Returns:
        ``(version_string, enriched_csv_path)``.
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    enriched_csv = out_dir / "icdc_enum_terms_for_verification_enriched.csv"

    version_string = discover_icdc_version(client)
    if not version_string:
        raise RuntimeError("Could not discover ICDC version from STS")

    print(f"Enrich: ICDC version {version_string!r}")
...
def run_verify(
    ...
    *,
    ...
    """
    Verify rows with non-empty ``term_value`` via :func:`verify_row`; write ICDC report CSV/MD.

    Returns:
        ``(report_csv, report_md, passed_count, total_rows_verified)``.
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    report_csv = out_dir / "icdc_term_endpoint_verification_report.csv"
    report_md = out_dir / "icdc_term_endpoint_verification_report.md"
    ...
    with open(report_md, "w", encoding="utf-8") as f:
        f.write("# ICDC term endpoint verification report\n\n")
        f.write(
            f"**Base URL:** `{base_url}`\n\n"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "ICDC term-by-value pipeline: extract from YAML, enrich via STS, verify /term/{value}, "
            "write CSV + Markdown under reports/term_value/ICDC/"
        )
    )
    parser.add_argument(
        "--yaml",
        type=Path,
        default=None,
        help=f"Path to icdc-model-props.yml (default: {default_yaml_path()})",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
    ...
    parser.add_argument(
        "--skip-extract",
        action="store_true",
        help="Skip extract; expect icdc_enum_terms_for_verification.csv in out-dir",
    )
    parser.add_argument(
        "--skip-enrich",
        action="store_true",
        help="Skip enrich; expect icdc_enum_terms_for_verification_enriched.csv in out-dir",
    )
    parser.add_argument(
        "--warn-only",
        action="store_true",
