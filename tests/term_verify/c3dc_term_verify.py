"""
C3DC term-by-value verification pipeline.

Subclass of :class:`~sts_test_framework.term_verify_pipeline.TermVerifyPipeline`.

Uses a flexible ``ENUM_HEADER_PATTERN`` regex and
:func:`~sts_test_framework.term_verify_utils.strip_inline_yaml_comment` + ``_strip_quotes``
per ``extract_c3dc_enum_properties.py``.
...
class C3DCTermVerify(TermVerifyPipeline):
    model_handle = "C3DC"
    csv_prefix = "c3dc"
    default_yaml_filename = "c3dc-model-props.yml"
    report_subdir = "C3DC"
