...
| `STS_PARALLEL_WORKERS` | Max models to run concurrently in `scripts/run_autogenerated_tests.py` (set to `1` for sequential)              | `4`                            |
...
- Runs the CLI once per model with `--model <handle>` and `--release`, **in parallel** by default (up to `STS_PARALLEL_WORKERS` models at a time).
...
| `STS_PARALLEL_WORKERS` | Max models to run concurrently. Set to `1` for sequential execution.                          | `4`                                                |
