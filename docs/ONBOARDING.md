...
Each model writes final reports as `{prefix}_term_endpoint_verification_report.csv` and `{prefix}_term_endpoint_verification_report.md` (prefixes: `ccdi`_, `c3dc_`, `ctdc_`, `icdc_`, `cds_`, `ccdi_dcc_`).
...
The launcher prints the URL (default **[http://localhost:5678](http://localhost:5678)**; if that port is in use, the next free port is chosen). Your browser should open automatically once the server is ready.
...
| **Performance** run (latency / throughput)                          | Newest `reports/perf/<ModelHandle>/perf_*.html` (and `.json`); see [§7.5](#75-performance-testing-concurrent-gets)                              |
...
Optional: `--fail-on-error-rate PCT` exits 1 if 5xx/network error rate exceeds *PCT*; slowness alone never fails.

**Output:** `reports/perf/<ModelHandle>/perf_<timestamp>.html` and `.json` — run summary (wall time, throughput, errors, slow count), global latency percentiles, histogram, per-`operation_id` table (sorted by p95), slowest requests, and full `raw_results` in JSON. 

Logs: `logs/perf_*.log` when using the shell script. 

This suite is **not** included in `run_full_suite.sh`.
