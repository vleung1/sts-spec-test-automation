...
| `CADSR_BASE_URL` | Root URL for the caDSR REST API (manual test `test_cadsr_designations_vs_sts_pvs.py` only; paths like `/DataElement/{publicId}`) | `https://cadsrapi.cancer.gov/rad/NCIAPI/1.0/api` |
...
**caDSR vs STS PVS (manual tests):** `tests/test_manual/test_cadsr_designations_vs_sts_pvs.py` (marker `cadsr_alt_pvs`) compares caDSR **Designations** names to STS **cde-pvs** and **model-pvs** responses. Case list: `data/cadsr_sts_pvs_cases.json`. Set **`CADSR_BASE_URL`** if not using the default public API host; **`STS_SSL_VERIFY`** applies to both STS and caDSR clients (`APIClient`). By default **every** designation **name** (all types) must appear as a STS `value`; set **`CADSR_DESIGNATION_TYPES`** (e.g. `MCL Alt Name`) to only require those types.
